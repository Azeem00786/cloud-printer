...
function getDevicePrintingRequired($db, $mac) {
    ...
    if ($results) {
        $row = $results->fetchArray(SQLITE3_ASSOC); // Fetch as associative array

        if ($row) {
            return array(intval($row['Printing']), intval($row['QueueID']), intval($row['DotWidth']));
        } else {
            return array(NULL, NULL, NULL); // No data found
        }
    } else {
        // Handle query error
        error_log("Database query failed: " . $db->lastErrorMsg());
        return array(NULL, NULL, NULL); // Return NULLs if query fails
    }
}
